...
public abstract class AbstractDependencyFilterMojo extends AbstractDependencyMojo {
     ...
     *
     * @since 2.0
     * @parameter default-value="java-source"
     */
     ...
     *
     * @since 2.0
     * @parameter default-value="${project.build.directory}/dependency-maven-plugin-markers"
     */
     ...
     *
     * @since 2.2
     * @parameter alias="mdep.prependGroupId" default-value="false"
     */
    ...
    /**
     * Method creates filters and filters the projects dependencies. This method also transforms the dependencies if classifier
     * is set. The dependencies are filtered in least specific to most specific order
     *
     * @param stopOnFailure
    ...
    protected DependencyStatusSets getDependencySets(boolean stopOnFailure) throws MojoExecutionException {
        ...
        filter.addFilter(new ClassifierFilter(DependencyUtil.cleanToBeTokenizedString(this.includeClassifiers), DependencyUtil
                .cleanToBeTokenizedString(this.excludeClassifiers)));

        filter.addFilter(new GroupIdFilter(DependencyUtil.cleanToBeTokenizedString(this.includeGroupIds), DependencyUtil
                .cleanToBeTokenizedString(this.excludeGroupIds)));

        filter.addFilter(new ArtifactIdFilter(DependencyUtil.cleanToBeTokenizedString(this.includeArtifactIds), DependencyUtil
