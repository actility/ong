...
typedef void (*PF_HTTP_RESPONSE_CB)(void *issuerHandle, transaction_t *transaction);

typedef struct httpClient_t
{
  void (*createReq)(struct httpClient_t *, char *transId);
  transaction_t * (*getTransaction)(struct httpClient_t *, char *transId);
  ...
  transactionsList_t *transactions;
  freeTransactionsList_t *freeTransactions;
  ...
  PF_HTTP_RESPONSE_CB responseCb;
  void *issuerHandle;
