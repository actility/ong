...
public final class ResourcesManager implements MonitoringListener, Serializable {
    ...
    private static final String PATH_APPLICATIONS = "/m2m/applications";
    private static final String PATH_APPLICATION_SYSTEM = "/m2m/applications/SYSTEM";
    private static final String PATH_CONTAINERS_SYSTEM = "/m2m/applications/SYSTEM/containers";
    private static final String PATH_CONTAINER_DESCRIPTOR = "/m2m/applications/SYSTEM/containers/DESCRIPTOR";
    private static final String PATH_CONTENT_INSTANCES_DESCRIPTOR = "/m2m/applications/SYSTEM/containers/DESCRIPTOR/contentInstances/";
    private static final String PATH_CONTAINER_RESOURCES = "/m2m/applications/SYSTEM/containers/resources";
    private static final String PATH_CONTAINER_HIGH_RATE_LOGS = "/m2m/applications/SYSTEM/containers/highRateLogs";
    private static final String PATH_CONTAINER_MEDIUM_RATE_LOGS = "/m2m/applications/SYSTEM/containers/mediumRateLogs";
    private static final String PATH_CONTAINER_LOW_RATE_LOGS = "/m2m/applications/SYSTEM/containers/lowRateLogs";
    private static final String PATH_CONTAINER_ALARM_LOGS = "/m2m/applications/SYSTEM/containers/alarmLogs";
    ...
    private final ResourcesGenerator resGen;
    private final MonitoringServiceLauncher monitoringLauncher;
            ...
            if (lastOperation != null && lastTarget != null) {
                ...
                case SongServletResponse.SC_CREATED:
                    if (lastOperation.equals(SongServletRequest.MD_CREATE)) {
                        if (lastTarget.equals(PATH_APPLICATION_SYSTEM)) {
                            this.sendCreateRequest(PATH_CONTAINERS_SYSTEM, PATH_CONTAINER_DESCRIPTOR,
                                    resGen.generateCreateDescriptorContainer());
                        } else if (lastTarget.equals(PATH_CONTAINER_DESCRIPTOR)) {
                            this.sendCreateRequest(PATH_CONTENT_INSTANCES_DESCRIPTOR, PATH_CONTENT_INSTANCES_DESCRIPTOR,
                                    resGen.generateCreateDescriptorContentInstance());
                        } else if (lastTarget.equals(PATH_CONTENT_INSTANCES_DESCRIPTOR)) {
                            this.sendCreateRequest(PATH_CONTAINERS_SYSTEM, PATH_CONTAINER_RESOURCES,
                                    resGen.generateCreateResourcesContainer());
                        } else if (lastTarget.equals(PATH_CONTAINER_RESOURCES)) {
                            this.sendCreateRequest(PATH_CONTAINERS_SYSTEM, PATH_CONTAINER_HIGH_RATE_LOGS,
                                    resGen.generateCreateHighRateLogsContainer());
                        } else if (lastTarget.equals(PATH_CONTAINER_HIGH_RATE_LOGS)) {
                            this.sendCreateRequest(PATH_CONTAINERS_SYSTEM, PATH_CONTAINER_MEDIUM_RATE_LOGS,
                                    resGen.generateCreateMediumRateLogsContainer());
                        } else if (lastTarget.equals(PATH_CONTAINER_MEDIUM_RATE_LOGS)) {
                            this.sendCreateRequest(PATH_CONTAINERS_SYSTEM, PATH_CONTAINER_LOW_RATE_LOGS,
                                    resGen.generateCreateLowRateLogsContainer());
                        } else if (lastTarget.equals(PATH_CONTAINER_LOW_RATE_LOGS)) {
                            this.sendCreateRequest(PATH_CONTAINERS_SYSTEM, PATH_CONTAINER_ALARM_LOGS,
                                    resGen.generateCreateAlarmLogsContainer());
                        } else if (lastTarget.equals(PATH_CONTAINER_ALARM_LOGS)) {
                ...
                case SongServletResponse.SC_NO_CONTENT:
                    if (lastOperation.equals(SongServletRequest.MD_UPDATE)) {
                        if (lastTarget.equals(PATH_APPLICATION_SYSTEM)) {
                            this.sendUpdateRequest(PATH_CONTAINER_DESCRIPTOR, resGen.generateUpdateDescriptorContainer());
                        } else if (lastTarget.equals(PATH_CONTAINER_DESCRIPTOR)) {
                            this.sendCreateRequest(PATH_CONTENT_INSTANCES_DESCRIPTOR, PATH_CONTENT_INSTANCES_DESCRIPTOR,
                                    resGen.generateCreateDescriptorContentInstance());
                        } else if (lastTarget.equals(PATH_CONTENT_INSTANCES_DESCRIPTOR)) {
                            this.sendUpdateRequest(PATH_CONTAINER_RESOURCES, resGen.generateUpdateResourcesContainer());
                        } else if (lastTarget.equals(PATH_CONTAINER_RESOURCES)) {
                            this.sendUpdateRequest(PATH_CONTAINER_HIGH_RATE_LOGS, resGen.generateUpdateHighRateLogsContainer());
                        } else if (lastTarget.equals(PATH_CONTAINER_HIGH_RATE_LOGS)) {
                            this.sendUpdateRequest(PATH_CONTAINER_MEDIUM_RATE_LOGS,
                                    resGen.generateUpdateMediumRateLogsContainer());
                        } else if (lastTarget.equals(PATH_CONTAINER_MEDIUM_RATE_LOGS)) {
                            this.sendUpdateRequest(PATH_CONTAINER_LOW_RATE_LOGS, resGen.generateUpdateLowRateLogsContainer());
                        } else if (lastTarget.equals(PATH_CONTAINER_LOW_RATE_LOGS)) {
                            this.sendUpdateRequest(PATH_CONTAINER_ALARM_LOGS, resGen.generateUpdateAlarmLogsContainer());
                        } else if (lastTarget.equals(PATH_CONTAINER_ALARM_LOGS)) {
                            this.launchMonitoringService();
                        }
                    }
                ...
                case SongServletResponse.SC_NOT_FOUND:
                    if (lastOperation.equals(SongServletRequest.MD_UPDATE)) {
                        if (lastTarget.equals(PATH_APPLICATION_SYSTEM)) {
                            this.sendCreateRequest(PATH_APPLICATIONS, PATH_APPLICATION_SYSTEM,
                                    resGen.generateCreateSystemApplication());
                        } else if (lastTarget.equals(PATH_CONTAINER_DESCRIPTOR)) {
                            this.sendCreateRequest(PATH_CONTAINERS_SYSTEM, PATH_CONTAINER_DESCRIPTOR,
                                    resGen.generateCreateDescriptorContainer());
                        } else if (lastTarget.equals(PATH_CONTAINER_RESOURCES)) {
                ...
                case SongServletResponse.SC_CONFLICT:
                    if (lastOperation.equals(SongServletRequest.MD_CREATE)) {
                        if (lastTarget.equals(PATH_APPLICATION_SYSTEM)) {
                            this.sendUpdateRequest(PATH_APPLICATION_SYSTEM, resGen.generateUpdateSystemApplication());
                        } else if (lastTarget.equals(PATH_CONTAINER_DESCRIPTOR)) {
                            this.sendUpdateRequest(PATH_CONTAINER_DESCRIPTOR, resGen.generateUpdateDescriptorContainer());
                        } else if (lastTarget.equals(PATH_CONTAINER_RESOURCES)) {
                            this.sendUpdateRequest(PATH_CONTAINER_RESOURCES, resGen.generateUpdateResourcesContainer());
     ...
     */
    private void sendCreateRequest(String path, String target, XoObject xobject) throws IllegalArgumentException {
        try {
            byte[] content = xobject.saveXml();
            sendRequest(SongServletRequest.MD_CREATE, path, content, target);
        } catch (XoException e) {
     ...
     * @param xobject
     */
    private void sendUpdateRequest(String path, XoObject xobject) throws IllegalArgumentException {
        try {
            byte[] content = xobject.saveXml();
            sendRequest(SongServletRequest.MD_UPDATE, path, content);
        } catch (XoException e) {
            LOG.error("sendUpdateRequest - fail to export XoObject as xml", e);
        } finally {
            Utils.releaseXoObjectFromMemory(xobject);
    ...
    private void sendRequest(String operation, String path, byte[] content, String target, boolean context) {
        try {
            ...
            timerService.createTimer(appSession,
                    ((Integer) configuration.getValue(Configuration.Name.TIME_BETWEEN_INIT/* , Integer.class */)).intValue(),
                    false, this);
        } catch (IOException e) {
            ...
            if (byteTab != null) {
                ...
                try {
                    ...
                    while (iterator.hasNext()) {
                        ...
                        if (xoName != null && xoName.equals("profileUrl") && xoType != null && xoType.equals("o:ref")) {
                            ...
                            try {
                                ...
                                SongURI reqEntity = this.generateUri("/m2m/applications/GA");
                                SongServletRequest songRequest = factory.createRequest(appSession,
                                        SongServletRequest.MD_RETRIEVE, reqEntity, toUri);
                                songRequest.setAttribute(SystemVersionServlet.REQUEST_CONTEXT, this);
                                songRequest.send();
                                ...
                                byte[] content = Utils.generateErrorContent(xoService, StatusCode.STATUS_BAD_REQUEST,
                                        "The given url isn't valid");
                                if (content != null) {
                    ...
                    if (retargetingResponse == null && reloadProfileRequest == null) {
                        retargetingResponse = request.createResponse(SongServletResponse.SC_BAD_REQUEST);
                        byte[] content = Utils.generateErrorContent(xoService, StatusCode.STATUS_BAD_REQUEST,
                                "Obix not contains the profile url");
                        if (content != null) {
                            retargetingResponse.setContent(content, "application/xml; charset=utf-8");
                        }
                    }
                    ...
                    retargetingResponse = request.createResponse(SongServletResponse.SC_BAD_REQUEST);
                    byte[] content = Utils.generateErrorContent(xoService, StatusCode.STATUS_BAD_REQUEST,
                            "Xml content is not formatted correctly");
                    if (content != null) {
                        retargetingResponse.setContent(content, "application/xml; charset=utf-8");
                    }
                }
    ...
    public void onReloadProfileResponse(SongServletResponse response) throws IOException {
        if (reloadProfileRequest != null) {
            ...
            if (status == 200) {
                if (response.getContentLength() > 0) {
                    ...
                    if (byteTab != null) {
                        try {
                            ...
                            List xoUrlList = xoUrl.getXoObjectListAttribute("[]");
                            configuration.saveConfig(xoUrlList);
                            Utils.releaseXoObjectFromMemory(xoUrl);
                ...
                byte[] content = Utils.generateErrorContent(xoService, response.getReasonPhrase(),
                        "The response given by the reload profile url");
                if (content != null) {
                    retargetingResponse.setContent(content, "application/xml; charset=utf-8");
                }
            }
