...
public class MonitoringManager implements Serializable{
    /**
     * 
     */
    ...
    private Map/*<Indicator>*/ indicators;
    private Map/*<Indicator>*/ statics;
    private List/*<Alarms>*/ alarms;
    private SystemInformationManager infoManager;
        ...
        for(int i=0; i<indicators.size();i++){
            ...
            switch(indicator.getRefreshInterval()){
                ...
                case 180:
                    ...
                    break;
                default:
                    LOG.error("The indicator "+indicator.getName()+" has not logging container (interval="+indicator.getRefreshInterval()+")");
                    break;
            }
        }
        try {
            ...
            if(type.startsWith("list<")&& type.endsWith(">")){
                if(type.length()>6){
                    ...
                    if(indicator.getValue() instanceof List){
                        ...
                        LOG.error(*/"isn't type a list ("+indicator.getValue().getClass()+")");
                    }
                }else{
    ...
    private XoObject exportObjToObix(String type, String name, Object value) throws XoException{
        if(type.equals("bool") || type.equals("int") || type.equals("real") || type.equals("str") || type.equals("enum") || type.equals("uri") || type.equals("abstime") || type.equals("reltime") || type.equals("date") || type.equals("time")){
...
//        indicators.put(Indicator.Name.CPU, new Indicator(Indicator.Name.CPU, , ));
        ...
        indicators.put(Indicator.Name.BAT_REMAINING_TIME, new Indicator(Indicator.Name.BAT_REMAINING_TIME, "list<reltime>", 180));
...
//        indicators.put(Indicator.Name.LRR_LINK_STATE, new Indicator(Indicator.Name.LRR_LINK_STATE, , ));
    ...
    private void initAlarms(){
        ...
        alarms.add(new Alarm(Alarm.Name.HIGH_LOAD_AVG_THRESHOLD, new String[]{Indicator.Name.LOAD_AVG_PT15M, Indicator.Name.NUM_CPU}, new AlarmExecution() {
            public List/*<AlarmExecutionResult>*/ execute(Configuration configuration, Map indicators) throws AlarmExecutionException {
                List results = new ArrayList();
                Indicator loadAvg = (Indicator) indicators.get(Indicator.Name.LOAD_AVG_PT15M);
                if(loadAvg==null){
                    throw new AlarmExecutionException("Indicator "+Indicator.Name.LOAD_AVG_PT15M+" is null",AlarmExecutionException.LEVEL_ERROR);
                }
                Object loadAvgValue = loadAvg.getValue();
                if(loadAvgValue== null || ! (loadAvgValue instanceof Float)){
                    throw new AlarmExecutionException("Value of indicator "+Indicator.Name.LOAD_AVG_PT15M+" is null or is not a Float",AlarmExecutionException.LEVEL_ERROR);
                }
                float load = ((Float) loadAvgValue).floatValue();

                Indicator numCpu = (Indicator) indicators.get(Indicator.Name.NUM_CPU);
                if(numCpu==null){
                    throw new AlarmExecutionException("Indicator "+Indicator.Name.NUM_CPU+" is null",AlarmExecutionException.LEVEL_ERROR);
                }
                Object numCpuValue = numCpu.getValue();
                if(numCpuValue== null || ! (numCpuValue instanceof Integer)){
                    throw new AlarmExecutionException("Value of indicator "+Indicator.Name.NUM_CPU+" is null or is not a Integer",AlarmExecutionException.LEVEL_ERROR);
                }
                int ncpu = ((Integer) numCpuValue).intValue();

                Float alarmLoadAvgThresholdCrit = (Float) configuration.getValue(Configuration.Name.ALARM_LOAD_AVG_THRESHOLD_CRIT/*, Float.class*/);
                if(alarmLoadAvgThresholdCrit == null){
                    throw new AlarmExecutionException("Configuration value "+Configuration.Name.ALARM_LOAD_AVG_THRESHOLD_CRIT+" cannot be null",AlarmExecutionException.LEVEL_ERROR);
                }

                Float alarmLoadAvgThresholdMaj = (Float) configuration.getValue(Configuration.Name.ALARM_LOAD_AVG_THRESHOLD_MAJ/*, Float.class*/);
                if(alarmLoadAvgThresholdMaj == null){
                    throw new AlarmExecutionException("Configuration value "+Configuration.Name.ALARM_LOAD_AVG_THRESHOLD_MAJ+" cannot be null",AlarmExecutionException.LEVEL_ERROR);
                }
        ...
        alarms.add(new Alarm(Alarm.Name.HIGH_RAM_THRESHOLD, new String[]{Indicator.Name.MEM_FREE,Indicator.Name.MEM_TOTAL}, new AlarmExecution() {
            public List execute(Configuration configuration, Map indicators) throws AlarmExecutionException {
                ...
                if(memFree==null){
                    throw new AlarmExecutionException("Indicator "+Indicator.Name.MEM_FREE+" is null",AlarmExecutionException.LEVEL_ERROR);
                }
                Object memFreeValue = memFree.getValue();
                if(memFreeValue==null || !(memFreeValue instanceof Integer)){
                    throw new AlarmExecutionException("Value of indicator "+Indicator.Name.MEM_FREE+" is null or is not an Integer",AlarmExecutionException.LEVEL_ERROR);
                }
                int free = ((Integer) memFreeValue).intValue();
                Indicator memTotal = (Indicator) indicators.get(Indicator.Name.MEM_TOTAL);
                if(memTotal==null){
                    throw new AlarmExecutionException("Indicator "+Indicator.Name.MEM_TOTAL+" is null",AlarmExecutionException.LEVEL_ERROR);
                }
                Object memTotalValue = memTotal.getValue();
                if(memTotalValue==null || !(memTotalValue instanceof Integer)){
                    throw new AlarmExecutionException("Value of indicator "+Indicator.Name.MEM_TOTAL+" is null or is not an Integer",AlarmExecutionException.LEVEL_ERROR);
                }
                int total = ((Integer) memTotalValue).intValue();
                Float alarmRamThresholdCrit = (Float) configuration.getValue(Configuration.Name.ALARM_RAM_THRESHOLD_CRIT/*, Float.class*/);
                if(alarmRamThresholdCrit == null){
                    throw new AlarmExecutionException("Configuration value "+Configuration.Name.ALARM_RAM_THRESHOLD_CRIT+" cannot be null",AlarmExecutionException.LEVEL_ERROR);
                }
                Float alarmRamThresholdMaj = (Float) configuration.getValue(Configuration.Name.ALARM_RAM_THRESHOLD_MAJ/*, Float.class*/);
                if(alarmRamThresholdMaj == null){
                    throw new AlarmExecutionException("Configuration value "+Configuration.Name.ALARM_RAM_THRESHOLD_MAJ+" cannot be null",AlarmExecutionException.LEVEL_ERROR);
                }
                float result = (float)free/total;
                if(result < alarmRamThresholdCrit.floatValue()){
                    results.add(new AlarmExecutionResult(Alarm.ALARM_STATE_CRITICAL,"There are only "+free+"KB of free ram"));
                }else if(result < alarmRamThresholdMaj.floatValue()){
                    results.add(new AlarmExecutionResult(Alarm.ALARM_STATE_MAJOR,"There are only "+free+"KB of free ram"));
                }else if(result >= alarmRamThresholdMaj.floatValue()){
                    results.add(new AlarmExecutionResult(Alarm.ALARM_STATE_CLEAR));
                }
                return results;
        ...
        alarms.add(new Alarm(Alarm.Name.HIGH_DISK_THRESHOLD, new String[]{Indicator.Name.DISK_PART_FREE, Indicator.Name.DISK_PART_SIZE}, new AlarmExecution() {
            public List execute(Configuration configuration, Map indicators) throws AlarmExecutionException {
                List results = new ArrayList();
                Indicator diskPartFree = (Indicator) indicators.get(Indicator.Name.DISK_PART_FREE);
                if(diskPartFree==null){
                    throw new AlarmExecutionException("Indicator "+Indicator.Name.DISK_PART_FREE+" is null",AlarmExecutionException.LEVEL_ERROR);
                }
                Object diskPartFreeValue = diskPartFree.getValue();
                if(diskPartFreeValue==null || !(diskPartFreeValue instanceof List)){
                    throw new AlarmExecutionException("Value of indicator "+Indicator.Name.DISK_PART_FREE+" is null or is not a List",AlarmExecutionException.LEVEL_ERROR);
                }
                ...
                if(diskPartSize==null){
                    throw new AlarmExecutionException("Indicator "+Indicator.Name.DISK_PART_SIZE+" is null",AlarmExecutionException.LEVEL_ERROR);
                }
                Object diskPartSizeValue = diskPartSize.getValue();
                if(diskPartSizeValue==null || !(diskPartSizeValue instanceof List)){
                    throw new AlarmExecutionException("Value of indicator "+Indicator.Name.DISK_PART_SIZE+" is null or is not a List",AlarmExecutionException.LEVEL_ERROR);
                }
                List partSize = ((List)diskPartSizeValue);
                if(partFree.size()!=partSize.size()){
                    throw new AlarmExecutionException("List size of indicator"+Indicator.Name.DISK_PART_FREE+"("+partFree.size()+") is different of list size of indicator"+Indicator.Name.DISK_PART_SIZE+"("+partSize.size()+")",AlarmExecutionException.LEVEL_ERROR);
                }
                Float alarmDiskThresholdCrit = (Float) configuration.getValue(Configuration.Name.ALARM_DISK_THRESHOLD_CRIT/*, Float.class*/);
                if(alarmDiskThresholdCrit == null){
                    throw new AlarmExecutionException("Configuration value "+Configuration.Name.ALARM_DISK_THRESHOLD_CRIT+" cannot be null",AlarmExecutionException.LEVEL_ERROR);
                }
                Float alarmDiskThresholdMaj = (Float) configuration.getValue(Configuration.Name.ALARM_DISK_THRESHOLD_MAJ/*, Float.class*/);
                if(alarmDiskThresholdMaj == null){
                    throw new AlarmExecutionException("Configuration value "+Configuration.Name.ALARM_DISK_THRESHOLD_MAJ+" cannot be null",AlarmExecutionException.LEVEL_ERROR);
                }
                for(int i=0;i<partFree.size();i++){
                    Object partFreeValue = partFree.get(i);
                    if(partFreeValue==null || !(partFreeValue instanceof Integer)){
                        throw new AlarmExecutionException("Value of indicator "+Indicator.Name.DISK_PART_FREE+"["+i+"] is null or is not an Integer",AlarmExecutionException.LEVEL_ERROR);
                    }
                    ...
                    if(partSizeValue==null || !(partSizeValue instanceof Integer)){
                        throw new AlarmExecutionException("Value of indicator "+Indicator.Name.DISK_PART_SIZE+"["+i+"] is null or is not an Integer",AlarmExecutionException.LEVEL_ERROR);
                    }
                    int size = ((Integer) partSizeValue).intValue();
                    float result = (float)free/size;
                    if(result < alarmDiskThresholdCrit.floatValue()){
                        results.add(new AlarmExecutionResult(Alarm.ALARM_STATE_CRITICAL,"There are only "+free+"KB of free space on disk partition",i));
                    }else if(result < alarmDiskThresholdMaj.floatValue()){
                        results.add(new AlarmExecutionResult(Alarm.ALARM_STATE_MAJOR,"There are only "+free+"KB of free space on disk partition",i));
                    }else if(result >= alarmDiskThresholdMaj.floatValue()){
        ...
        alarms.add(new Alarm(Alarm.Name.HIGH_DISK_IO_THRESHOLD, new String[]{Indicator.Name.DISK_READ_IO, Indicator.Name.DISK_WRITE_IO}, new AlarmExecution() {
            public List execute(Configuration configuration, Map indicators) throws AlarmExecutionException {
                List results = new ArrayList();
                Float alarmDiskIoThresholdMaj = (Float) configuration.getValue(Configuration.Name.ALARM_DISK_IO_THRESHOLD_MAJ/*, Float.class*/);
                if(alarmDiskIoThresholdMaj == null){
                    throw new AlarmExecutionException("Configuration value "+Configuration.Name.ALARM_DISK_IO_THRESHOLD_MAJ+" cannot be null",AlarmExecutionException.LEVEL_CLEAR);
                }
                Float alarmDiskIoThresholdCrit = (Float) configuration.getValue(Configuration.Name.ALARM_DISK_IO_THRESHOLD_CRIT/*, Float.class*/);
                if(alarmDiskIoThresholdCrit == null){
                    throw new AlarmExecutionException("Configuration value "+Configuration.Name.ALARM_DISK_IO_THRESHOLD_CRIT+" cannot be null",AlarmExecutionException.LEVEL_CLEAR);
                }
                Float maxRateDiskIo = (Float) configuration.getValue(Configuration.Name.MAX_RATE_DISK_IO/*, Float.class*/);
                if(maxRateDiskIo == null){
                    throw new AlarmExecutionException("Configuration value "+Configuration.Name.MAX_RATE_DISK_IO+" cannot be null",AlarmExecutionException.LEVEL_CLEAR);
                }
                Indicator diskReadIo = (Indicator) indicators.get(Indicator.Name.DISK_READ_IO);
                if(diskReadIo==null){
                    throw new AlarmExecutionException("Indicator "+Indicator.Name.DISK_READ_IO+" is null",AlarmExecutionException.LEVEL_ERROR);
                }
                Object diskReadIoValue = diskReadIo.getValue();
                if(diskReadIoValue==null || !(diskReadIoValue instanceof List)){
                    throw new AlarmExecutionException("Value of indicator "+Indicator.Name.DISK_READ_IO+" is null or is not a List",AlarmExecutionException.LEVEL_ERROR);
                }
                List readsIo = ((List)diskReadIoValue);
                Indicator diskWriteIo = (Indicator) indicators.get(Indicator.Name.DISK_WRITE_IO);
                if(diskWriteIo==null){
                    throw new AlarmExecutionException("Indicator "+Indicator.Name.DISK_WRITE_IO+" is null",AlarmExecutionException.LEVEL_ERROR);
                }
                Object diskWriteIoValue = diskWriteIo.getValue();
                if(diskWriteIoValue==null || !(diskWriteIoValue instanceof List)){
                    throw new AlarmExecutionException("Value of indicator "+Indicator.Name.DISK_WRITE_IO+" is null or is not a List",AlarmExecutionException.LEVEL_ERROR);
                }
                List writesIo = ((List)diskWriteIoValue);
                if(writesIo.size()!=readsIo.size()){
                    throw new AlarmExecutionException("List size of indicator"+Indicator.Name.DISK_WRITE_IO+"("+writesIo.size()+") is different of list size of indicator"+Indicator.Name.DISK_READ_IO+"("+readsIo.size()+")",AlarmExecutionException.LEVEL_ERROR);
                }
                for(int i=0; i<writesIo.size();i++){
                    Object writeIoValue = writesIo.get(i);
                    if(writeIoValue==null || !(writeIoValue instanceof Float)){
                        throw new AlarmExecutionException("Value of indicator "+Indicator.Name.DISK_WRITE_IO+"["+i+"] is null or is not a Float",AlarmExecutionException.LEVEL_ERROR);
                    }
                    ...
                    if(readIoValue==null || !(readIoValue instanceof Float)){
                        throw new AlarmExecutionException("Value of indicator "+Indicator.Name.DISK_READ_IO+"["+i+"] is null or is not a Float",AlarmExecutionException.LEVEL_ERROR);
                    }
                    float readIo = ((Float) readIoValue).floatValue();
                    if(readIo/maxRateDiskIo.floatValue() > alarmDiskIoThresholdCrit.floatValue() || writeIo/maxRateDiskIo.floatValue() > alarmDiskIoThresholdCrit.floatValue()){
                        results.add(new AlarmExecutionResult(Alarm.ALARM_STATE_CRITICAL,i));
                    }else if(readIo/maxRateDiskIo.floatValue() > alarmDiskIoThresholdMaj.floatValue() || writeIo/maxRateDiskIo.floatValue() > alarmDiskIoThresholdMaj.floatValue()){
...
//            }
//        }, 1440, 1440,true));
        alarms.add(new Alarm(Alarm.Name.POWER_SUPPLY_FAILURE, new String[]{Indicator.Name.MAIN_PWR_ON}, new AlarmExecution() {
            public List execute(Configuration configuration, Map indicators) throws AlarmExecutionException {
                List results = new ArrayList();
                Indicator mainPwrOn = (Indicator) indicators.get(Indicator.Name.MAIN_PWR_ON);
                if(mainPwrOn==null){
                    throw new AlarmExecutionException("Indicator "+Indicator.Name.MAIN_PWR_ON+" is null",AlarmExecutionException.LEVEL_ERROR);
                }
                Object mainPwrOnValue = mainPwrOn.getValue();
                if(mainPwrOnValue== null || ! (mainPwrOnValue instanceof Boolean)){
                    throw new AlarmExecutionException("Value of indicator "+Indicator.Name.MAIN_PWR_ON+" is null or is not a Boolean",AlarmExecutionException.LEVEL_ERROR);
                }
        ...
        alarms.add(new Alarm(Alarm.Name.HIGH_NET_IO_THRESHOLD, new String[]{Indicator.Name.NET_RECV_IO,Indicator.Name.NET_SENT_IO}, new AlarmExecution() {
            public List execute(Configuration configuration, Map indicators) throws AlarmExecutionException {
                List results = new ArrayList();
                Float alarmNetIoThresholdMaj = (Float) configuration.getValue(Configuration.Name.ALARM_NET_IO_THRESHOLD_MAJ/*, Float.class*/);
                if(alarmNetIoThresholdMaj == null){
                    throw new AlarmExecutionException("Configuration value "+Configuration.Name.ALARM_NET_IO_THRESHOLD_MAJ+" cannot be null",AlarmExecutionException.LEVEL_CLEAR);
                }
                Float alarmNetIoThresholdCrit = (Float) configuration.getValue(Configuration.Name.ALARM_NET_IO_THRESHOLD_CRIT/*, Float.class*/);
                if(alarmNetIoThresholdCrit == null){
                    throw new AlarmExecutionException("Configuration value "+Configuration.Name.ALARM_NET_IO_THRESHOLD_CRIT+" cannot be null",AlarmExecutionException.LEVEL_CLEAR);
                }
                Float maxRateNetIo = (Float) configuration.getValue(Configuration.Name.MAX_RATE_NET_IO/*, Float.class*/);
                if(maxRateNetIo == null){
                    throw new AlarmExecutionException("Configuration value "+Configuration.Name.MAX_RATE_NET_IO+" cannot be null",AlarmExecutionException.LEVEL_CLEAR);
                }
                Indicator netRecvIo = (Indicator) indicators.get(Indicator.Name.NET_RECV_IO);
                if(netRecvIo==null){
                    throw new AlarmExecutionException("Indicator "+Indicator.Name.NET_RECV_IO+" is null",AlarmExecutionException.LEVEL_ERROR);
                }
                Object netRecvIoValue = netRecvIo.getValue();
                if(netRecvIoValue==null || !(netRecvIoValue instanceof List)){
                    throw new AlarmExecutionException("Value of indicator "+Indicator.Name.NET_RECV_IO+" is null or is not a List",AlarmExecutionException.LEVEL_ERROR);
                }
                List recvsIo = ((List)netRecvIoValue);
                Indicator netSentIo = (Indicator) indicators.get(Indicator.Name.NET_SENT_IO);
                if(netSentIo==null){
                    throw new AlarmExecutionException("Indicator "+Indicator.Name.NET_SENT_IO+" is null",AlarmExecutionException.LEVEL_ERROR);
                }
                Object netSentIoValue = netSentIo.getValue();
                if(netSentIoValue==null || !(netSentIoValue instanceof List)){
                    throw new AlarmExecutionException("Value of indicator "+Indicator.Name.NET_SENT_IO+" is null or is not a List",AlarmExecutionException.LEVEL_ERROR);
                }
                List sentsIo = ((List)netSentIoValue);
                if(sentsIo.size()!=recvsIo.size()){
                    throw new AlarmExecutionException("List size of indicator"+Indicator.Name.NET_SENT_IO+"("+sentsIo.size()+") is different of list size of indicator"+Indicator.Name.NET_RECV_IO+"("+recvsIo.size()+")",AlarmExecutionException.LEVEL_ERROR);
                }
                for(int i=0; i<sentsIo.size();i++){
                    Object sentIoValue = sentsIo.get(i);
                    if(sentIoValue==null || !(sentIoValue instanceof Float)){
                        throw new AlarmExecutionException("Value of indicator "+Indicator.Name.NET_SENT_IO+"["+i+"] is null or is not a Float",AlarmExecutionException.LEVEL_ERROR);
                    }
                    ...
                    if(recvIoValue==null || !(recvIoValue instanceof Float)){
                        throw new AlarmExecutionException("Value of indicator "+Indicator.Name.NET_RECV_IO+"["+i+"] is null or is not a Float",AlarmExecutionException.LEVEL_ERROR);
                    }
                    float recvIo = ((Float) recvIoValue).floatValue();
                    if(recvIo/maxRateNetIo.floatValue() > alarmNetIoThresholdCrit.floatValue() || sentIo/maxRateNetIo.floatValue() > alarmNetIoThresholdCrit.floatValue()){
                        results.add(new AlarmExecutionResult(Alarm.ALARM_STATE_CRITICAL,i));
                    }else if(recvIo/maxRateNetIo.floatValue() > alarmNetIoThresholdMaj.floatValue() || sentIo/maxRateNetIo.floatValue() > alarmNetIoThresholdMaj.floatValue()){
                        results.add(new AlarmExecutionResult(Alarm.ALARM_STATE_MAJOR,i));
                    }else if(recvIo/maxRateNetIo.floatValue() <= alarmNetIoThresholdMaj.floatValue() || sentIo/maxRateNetIo.floatValue() <= alarmNetIoThresholdMaj.floatValue()){
                        results.add(new AlarmExecutionResult(Alarm.ALARM_STATE_CLEAR,i));
                    }
...
//        }, 15, 60,true));
        alarms.add(new Alarm(Alarm.Name.HIGH_TEMP_THRESHOLD, new String[]{Indicator.Name.TEMP_CORE}, new AlarmExecution() {
            public List execute(Configuration configuration, Map indicators) throws AlarmExecutionException {
                ...
                Float alarmTempThresholdCrit = (Float) configuration.getValue(Configuration.Name.ALARM_TEMP_THRESHOLD_CRIT/*, Float.class*/);
                if(alarmTempThresholdCrit == null){
                    throw new AlarmExecutionException("Configuration value "+Configuration.Name.ALARM_TEMP_THRESHOLD_CRIT+" cannot be null",AlarmExecutionException.LEVEL_CLEAR);
                }
                
                Float alarmTempThresholdMaj = (Float) configuration.getValue(Configuration.Name.ALARM_TEMP_THRESHOLD_MAJ/*, Float.class*/);
                if(alarmTempThresholdMaj == null){
                    throw new AlarmExecutionException("Configuration value "+Configuration.Name.ALARM_TEMP_THRESHOLD_MAJ+" cannot be null",AlarmExecutionException.LEVEL_CLEAR);
                }
                Indicator tempCore = (Indicator) indicators.get(Indicator.Name.TEMP_CORE);
                if(tempCore==null){
                    throw new AlarmExecutionException("Indicator "+Indicator.Name.TEMP_CORE+" is null",AlarmExecutionException.LEVEL_ERROR);
                }
                Object tempCoreValue = tempCore.getValue();
                if(tempCoreValue==null || !(tempCoreValue instanceof List)){
                    throw new AlarmExecutionException("Value of indicator "+Indicator.Name.TEMP_CORE+" is null or is not a List",AlarmExecutionException.LEVEL_ERROR);
                }
                ...
                for(int i=0;i<tempsCore.size();i++){
                    ...
                    if(tempValue==null || !(tempValue instanceof Float) ){
                        throw new AlarmExecutionException("Value of indicator "+Indicator.Name.TEMP_CORE+"["+i+"] is null or is not a Float",AlarmExecutionException.LEVEL_ERROR);
                    }
                    float temp = ((Float) tempValue).floatValue();
                    if(temp > alarmTempThresholdCrit.floatValue()){
                        results.add(new AlarmExecutionResult(Alarm.ALARM_STATE_CRITICAL,"Cpu's temperature is too high: "+temp+"°C",i));
                    }else if(temp > alarmTempThresholdMaj.floatValue()){
                        results.add(new AlarmExecutionResult(Alarm.ALARM_STATE_MAJOR,"Cpu's temperature is too high: "+temp+"°C",i));
                    }else if(temp <= alarmTempThresholdMaj.floatValue()){
                        results.add(new AlarmExecutionResult(Alarm.ALARM_STATE_CLEAR,i));
                    }
                }
        ...
        alarms.add(new Alarm(Alarm.Name.TIME_SYNC_ERROR, new String[]{Indicator.Name.TIME_SYNC}, new AlarmExecution() {
            public List execute(Configuration configuration, Map indicators) throws AlarmExecutionException {
                ...
                if(timeSync==null){
                    throw new AlarmExecutionException("Indicator "+Indicator.Name.TIME_SYNC+" is null",AlarmExecutionException.LEVEL_ERROR);
                }
                Object timeSyncValue = timeSync.getValue();
                if(timeSyncValue== null || ! (timeSyncValue instanceof Boolean)){
                    throw new AlarmExecutionException("Value of indicator "+Indicator.Name.TIME_SYNC+" is null or is not a Boolean",AlarmExecutionException.LEVEL_ERROR);
                }
...
//            private boolean start = true;
            public List execute(Configuration configuration, Map indicators) throws AlarmExecutionException {
                ...
                if(uptime==null){
                    throw new AlarmExecutionException("Indicator "+Indicator.Name.UPTIME+" is null",AlarmExecutionException.LEVEL_ERROR);
                }
                Object uptimeValue = uptime.getValue();
                if(uptimeValue== null || ! (uptimeValue instanceof String)){
                    throw new AlarmExecutionException("Value of indicator "+Indicator.Name.UPTIME+" is null or is not a String",AlarmExecutionException.LEVEL_ERROR);
                }
                ...
                try {
                    ...
                    throw new AlarmExecutionException("Value of indicator "+Indicator.Name.UPTIME+"(val:"+strUptime+") is not a duration",AlarmExecutionException.LEVEL_ERROR);
                }
                return results;
            }
        }, 15, 15,true));
