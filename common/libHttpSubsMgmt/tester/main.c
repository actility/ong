...
int DoArg(int argc, char *argv[])
{
  ...
  // last argv element must be <server-FQDN> mandatory param
  if (!argv[optind])
  {
    fprintf(stderr, "*** missing parameter <server-FQDN>\n");
    usage(argv[0], fmtgetopt);
    exit(1);
  }
...
int main(int argc, char *argv[])
{
  ...
  AdmTcpInit();
  if (!httpSubsInit(argv[optind], 9191, 60, 5, 10)) return 1;
