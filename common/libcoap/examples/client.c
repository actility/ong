...
coap_new_request(coap_context_t *ctx, method_t m, coap_list_t *options ) {
  ...
  if (payload.length) {
    if ((flags & FLAGS_BLOCK) == 0) {
      if (!coap_add_data(pdu, payload.length, payload.s)) {
			...
			block.szx = 6;
			coap_write_block_opt(&block, COAP_OPTION_BLOCK1, pdu, payload.length);
...
void copy_options (coap_pdu_t *from, coap_pdu_t *to) {
	...
	while	(coap_option_next(&opt_iter)) {
		coap_add_option(to, opt_iter.type, 
...
message_handler(struct coap_context_t  *ctx, 
  ...
  default:
	...
	if (coap_get_block(received, COAP_OPTION_BLOCK1, &b)) {
printf ("Block case C\n");
		// send next block
		coap_pdu_t *pdu  = coap_new_pdu();
	  ...
	  for (option = optlist; option; option = option->next ) {
		...
		b.szx	= block.szx;
		coap_write_block_opt(&b, COAP_OPTION_BLOCK1, pdu, payload.length);
		coap_add_block(pdu, payload.length, payload.s,
			 b.num, b.szx);
...
main(int argc, char **argv) {
  ...
  while ((opt = getopt(argc, argv, "Nb:e:f:g:m:p:s:t:o:v:A:B:O:P:T:S:")) != -1) {
    switch (opt) {
    case 'b' :
...
void stress (coap_context_t *ctx, coap_address_t *dst, int nb);
  stress (ctx, &dst, Stress);
#if 0
