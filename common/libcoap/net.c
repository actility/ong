...
#ifdef IPV6_V6ONLY
        if (listen_addr->addr.sa.sa_family == PF_INET6) {
                int on = 0;
                if (setsockopt(c->sockfd, IPPROTO_IPV6, IPV6_V6ONLY, (char *)&on, sizeof(on)) == -1)
                        coap_log (LOG_WARN, "setsockopt IPPROTO_IPV6\n");
        }
#endif
...
static char *coap_get_ip_str(const struct sockaddr *sa, char *s, size_t maxlen) {
    switch(sa->sa_family) {
        case AF_INET:
...
*/
	...
	char tmp[100];
	coap_get_ip_str(&peer->addr.sa, tmp, sizeof(tmp));
	sprintf (tmp+strlen(tmp), ",%d", peer->addr.sin.sin_port);
	coap_hash (tmp, strlen(tmp), h);
...
coap_send_impl(coap_context_t *context, 
  ...
  if (bytes_written >= 0) {
    ...
    coap_log(LOG_CRIT, "coap_send: sendto errno %d [%s]\n", errno, strerror(errno));
  }
...
coap_retransmit( coap_context_t *context, coap_queue_t *node ) {
  ...
  debug("** removed transaction due to timeout%d\n", node->id);

	handle_response(context, node, NULL);
...
#else /* WITH_CONTIKI */
  if (!buf)
    buf = coap_malloc(coap_get_max_pdu_size());
  pdu = (coap_hdr_t *)buf;
#endif /* WITH_CONTIKI */
...
#ifndef WITH_CONTIKI
  bytes_read = recvfrom(ctx->sockfd, buf, coap_get_max_pdu_size(), 0,
...
wellknown_response(coap_context_t *context, coap_pdu_t *request) {
  ...
  resp = coap_pdu_init(request->hdr->type == COAP_MESSAGE_CON 
		       ...
		       request->hdr->id, coap_get_max_pdu_size());
  if (!resp)
    return NULL;
...
handle_request(coap_context_t *context, coap_queue_t *node) {      
  ...
  // ACT : handle all requests
  if (context->request_handler) {
    response = coap_pdu_init(node->pdu->hdr->type == COAP_MESSAGE_CON 
			     ? COAP_MESSAGE_ACK
			     : COAP_MESSAGE_NON,
			     0, node->pdu->hdr->id, coap_get_max_pdu_size());

    context->request_handler(context, &node->remote, node->pdu, response);
...
//debug ("Entering coap_dispatch\n");
  ...
  while ( context->recvqueue ) {
    ...
    rcvd->next = NULL;
//debug ("dequeue %lx type=%d\n", rcvd, rcvd->pdu->hdr->type);
...
/*
ACT : empty message is not an invalid one !
      else
	debug("dropped message with invalid code %d\n", rcvd->pdu->hdr->code);
*/
    }
