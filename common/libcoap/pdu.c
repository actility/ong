...
coap_pdu_init(unsigned char type, unsigned char code, 
  ...
  assert(size <= coap_get_max_pdu_size());
  /* Size must be large enough to fit the header. */
  if (size < sizeof(coap_hdr_t) || size > coap_get_max_pdu_size())
    return NULL;
...
#ifndef WITH_CONTIKI
  pdu = coap_pdu_init(0, 0, ntohs(COAP_INVALID_TID), coap_get_max_pdu_size());
#else /* WITH_CONTIKI */
  pdu = coap_pdu_init(0, 0, uip_ntohs(COAP_INVALID_TID), coap_get_max_pdu_size());
#endif /* WITH_CONTIKI */
...
#ifndef NDEBUG
 coap_log(LOG_DEBUG, "coap_add_data: cannot add: data too large for PDU\n");
#endif
    return 0;
  }
...
static size_t coap_max_pdu_size = COAP_MAX_PDU_SIZE;

void
coap_set_max_pdu_size(size_t aSize) {
	coap_max_pdu_size	= aSize;
}

size_t
coap_get_max_pdu_size() {
