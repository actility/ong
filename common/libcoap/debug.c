...
coap_show_pdu(const coap_pdu_t *pdu) {
  static char buf[64*1024]; /* need some space for output creation */

  sprintf (buf, "v:%d t:%d oc:%d c:%d id:%u", 
  ...
  if (pdu->hdr->optcnt) {
	...
	sprintf(buf+strlen(buf), " %d:", opt_iter.type);
      print_readable(COAP_OPT_VALUE(opt_iter.option), 
			 COAP_OPT_LENGTH(opt_iter.option), 
			 (unsigned char *)(buf+strlen(buf)), sizeof(buf), 0 );
    }
  }
/*  
...
#ifndef WITH_CONTIKI
void 
coap_log_default(coap_log_t level, const char *format, ...) {
  char timebuf[32];
  coap_tick_t now;
  va_list ap;
  FILE *log_fd;
...
void (*coap_log_impl)(coap_log_t level, const char *format, ...) = coap_log_default;

void
coap_set_log_impl(void (*fct)(coap_log_t level, const char *format, ...)) {
	coap_log_impl = fct;
}
