...
static void header_t_init(header_t *This)
{
  RTL_TRDBG(TRACE_DEBUG, "header_t::init\n");
...
header_t *new_header_t(char * name)
{
  ...
  RTL_TRDBG(TRACE_DEBUG, "new header_t (name:%s) (This:%0.8p)\n", This->name, This);
  return This;
}
...
{
  RTL_TRDBG(TRACE_DEBUG, "header_t::newFree (name:%s)\n", This->name);
  free(This->name);
  if (This->values)
  {
    free(This->values);
...
{
  RTL_TRDBG(TRACE_DEBUG, "header_t::addHeader (name:%s) (value:%s)\n",
    This->name, value);
  if ((!value) || (!*value))
  {
    RTL_TRDBG(TRACE_ERROR, "header_t::addHeader: error - no value provided"
      " (name:%s)\n", This->name);
    return;
  }
