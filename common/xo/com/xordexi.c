...
void usage()
{
	fprintf(stderr,"xordexi.x [-schema=f1.exs,f2.exs ...][-r f1.xor ... -r fN.xor] -[d|m] f1.exi ... -[d|m] fN.exi\n") ;
	fflush(stderr) ;
}
...
//	char	*hd  = NULL ;
	...
	for	( nb = 1 ; nb < argc ; nb++ )
	{
		...
		if	( memcmp(pt,"-schema",7) == 0 )
		{
			if	(pt[7] == '=')
				XoLoadEXISchemas(pt+8);
			else
				XoLoadEXISchemaDir (".");
			continue;
		}
		...
		switch	(nexttype)
		{
			...
			case	'd':		/* donnees */
				...
				XoLoadNameSpaceDir(".");
				//obj	= XoLoad(f) ;
				obj	= XoReadExiEx(file, NULL, flags, &err);
				if	( !obj )
				{
			...
			case	'm':		/* donnees */
				...
				obj	= XoReadExiMem(mem, st.st_size, NULL, flags, &err);
				if	( !obj )
				{
					fprintf(stderr,"\tload obj err : %s\n",
						file) ;	
