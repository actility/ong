...
static  char  *LoadConfig(char *fconfig, char *pref)
{
  ...
  RTL_TRDBG(TRACE_ERROR, "load config cluster '%s' from '%s'\n", pref, fconfig);
...
static  char  *FindInDir(char *dir, unsigned int cluster)
{
  ...
  RTL_TRDBG(TRACE_FULL, "search config cluster '%s' in '%s'\n", pref, dir);
  ...
  {
    RTL_TRDBG(TRACE_FULL, "cmp config cluster '%s' with '%s'\n", pref, rel);
