...
char *printVariable(iec_variable_t *var) {
	...
	case STRINGtype :
		strncat (tmp, var->value.STRINGvar.body, var->value.STRINGvar.len);
...
iec_fctinfos_t *searchStdFunc (char *name) {
	return rtl_htblGet(g_StdFunc, name);
}
