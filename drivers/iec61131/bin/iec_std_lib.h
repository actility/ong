 ...
 * Changes 2012 by Denis Froschauer (denis.froschauer@actility.com)
 *
...
static inline void __iec_error(char *msg) {
  /* TODO... */
  fprintf(stderr, "IEC 61131-3 runtime error. %s\n", msg);
  /*exit(1);*/
}
...
//  ts.tv_nsec = sign * (long int)((total_sec - ts.tv_sec)*1e9);
  ts.tv_nsec = sign * (long int)((total_sec - floor(total_sec))*1e9); // Actility
...
static inline IEC_TIMESPEC __tod_to_timespec(double seconds, double minutes, double hours) {
  ...
  broken_down_time.tm_mday = now->tm_mday;
  broken_down_time.tm_mon = now->tm_mon;
  broken_down_time.tm_year = now->tm_year;
...
static inline IEC_TIMESPEC __dt_to_timespec(double seconds,  double minutes, double hours, int day, int month, int year) {
  ...
  broken_down_time.tm_hour = 0;
  broken_down_time.tm_isdst = -1;
  broken_down_time.tm_mday = (day < 0) ? now->tm_mday : day;
  broken_down_time.tm_mon = (month < 0) ? now->tm_mon : month-1;
  broken_down_time.tm_year = (year < 0) ? now->tm_year : year-1900;
...
static inline STRING __tod_to_string(TOD IN){
    ...
    res = __INIT_STRING;
    seconds = IN.tv_sec;
    // Actility 25/7/2013 : localtime replace by gmtime
...
static inline DATE __date_and_time_to_date(DT IN){return (DATE){IN.tv_sec - (IN.tv_sec % 86400), 0};}
...
#else
#define __convert_type(from_TYPENAME,to_TYPENAME, oper) \
static inline void from_TYPENAME##_TO_##to_TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(to_TYPENAME)\
  RET->p.to_TYPENAME##var = (to_TYPENAME)oper(in->from_TYPENAME##var);\
  RET->t = to_TYPENAME##type;\
}
#endif
...
#else
#define __convert_num_to_bool(TYPENAME) \
static inline void TYPENAME##_TO_BOOL(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(BOOL)\
  RET->p.BOOLvar = in->TYPENAME##var == 0 ? 0 : 1;\
  RET->t = BOOLtype;\
}
#endif
...
static inline void TYPENAME##_TO_BOOL(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(BOOL)\
  RET->p.BOOLvar = ((in->TYPENAME##var.tv_sec == 0) && (in->TYPENAME##var.tv_nsec == 0)) ? 0 : 1;\
  RET->t = BOOLtype;\
}
#endif
...
#else
#define __iec_(to_TYPENAME,from_TYPENAME) \
static inline void TRUNC__##to_TYPENAME##__##from_TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(to_TYPENAME)\
  from_TYPENAME op = in++->from_TYPENAME##var;\
  RET->p.to_TYPENAME##var = __move_##to_TYPENAME(op);\
  RET->t = to_TYPENAME##type;\
}
#endif
...
#else
#define __iec_(to_TYPENAME,from_TYPENAME) \
static inline void from_TYPENAME##_TO_BCD_##to_TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(to_TYPENAME)\
  from_TYPENAME op = in++->from_TYPENAME##var;\
  RET->p.to_TYPENAME##var = (to_TYPENAME)__uint_to_bcd(op);\
  RET->t = to_TYPENAME##type;\
}
#endif
...
#else
#define __iec_(to_TYPENAME,from_TYPENAME) \
static inline void from_TYPENAME##_BCD_TO_##to_TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(to_TYPENAME)\
  from_TYPENAME op = in++->from_TYPENAME##var;\
  RET->p.to_TYPENAME##var = (to_TYPENAME)__bcd_to_uint(op);\
  RET->t = to_TYPENAME##type;\
}
#endif
...
#define __numeric(fname,TYPENAME, FUNC) \
/* explicitly typed function */\
static inline void fname##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(TYPENAME)\
  RET->p.TYPENAME##var = FUNC(in->TYPENAME##var);\
  RET->t = TYPENAME##type;\
}\
/* overloaded function */\
static inline void fname##_##TYPENAME##__##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  fname##TYPENAME(EN_ENO, RET, count, in);\
}
#endif
...
#define __abs_signed(TYPENAME) \
static inline void ABS_##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(TYPENAME)\
  TYPENAME op = in++->TYPENAME##var;\
  if (op < 0) op=-op;\
  RET->p.TYPENAME##var = op;\
  RET->t = TYPENAME##type;\
}\
static inline void ABS__##TYPENAME##__##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  ABS_##TYPENAME(EN_ENO, RET, count, in);\
}

#define __abs_unsigned(TYPENAME) \
static inline void ABS_##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(TYPENAME)\
  RET->p.TYPENAME##var = in++->TYPENAME##var;\
  RET->t = TYPENAME##type;\
}\
static inline void ABS__##TYPENAME##__##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  ABS_##TYPENAME(EN_ENO, RET, count, in);\
...
#define __arith_expand(fname,TYPENAME, OP)\
static inline void fname(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(TYPENAME)\
  UINT i;\
  TYPENAME op = in++->TYPENAME##var;\
  for (i = 1; i < count; i++) op = op OP in++->TYPENAME##var;\
  RET->p.TYPENAME##var = op;\
  RET->t = TYPENAME##type;\
}
...
static inline void fname##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(TYPENAME)\
  TYPENAME op1 = in++->TYPENAME##var;\
  TYPENAME op2 = in++->TYPENAME##var;\
  RET->p.TYPENAME##var = op1 OP op2;\
  RET->t = TYPENAME##type;\
}\
/* overloaded function */\
static inline void fname##_##TYPENAME##__##TYPENAME##__##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  return fname##TYPENAME(EN_ENO, RET, count, in);\
}
...
#define __div(TYPENAME)\
static inline void DIV_##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TYPENAME op1 = in++->TYPENAME##var;\
  TYPENAME op2 = in++->TYPENAME##var;\
  TEST_EN_COND(TYPENAME, op2 == 0)\
  RET->p.TYPENAME##var = op1 / op2;\
  RET->t = TYPENAME##type;\
}\
static inline void DIV_##_##TYPENAME##__##TYPENAME##__##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  return DIV_##TYPENAME(EN_ENO, RET, count, in);\
}
#endif
__ANY_NUM(__div)
...
#define __mod(TYPENAME)\
static inline void MOD_##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(TYPENAME)\
  TYPENAME op1 = in++->TYPENAME##var;\
  TYPENAME op2 = in++->TYPENAME##var;\
  if (op2 == 0) RET->p.TYPENAME##var = 0;\
  RET->p.TYPENAME##var = op1 % op2;\
  RET->t = TYPENAME##type;\
}\
static inline void MOD__##TYPENAME##__##TYPENAME##__##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  return MOD_##TYPENAME(EN_ENO, RET, count, in);\
}
#endif
__ANY_INT(__mod)
...
#define __iec_(in1_TYPENAME,in2_TYPENAME) \
static inline void EXPT__##in1_TYPENAME##__##in1_TYPENAME##__##in2_TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(in1_TYPENAME)\
  in1_TYPENAME IN1 = in++->in1_TYPENAME##var;\
  in2_TYPENAME IN2 = in++->in2_TYPENAME##var;\
  RET->p.in1_TYPENAME##var = pow(IN1, IN2);\
  RET->t = in1_TYPENAME##type;\
}
#endif
...
#define __iec_(TYPENAME) \
static inline void MOVE_##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(TYPENAME)\
  RET->p.TYPENAME##var = in->TYPENAME##var;\
  RET->t = TYPENAME##type;\
}
...
#define __iec_(TYPENAME) \
static inline void MOVE__##TYPENAME##__##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(TYPENAME)\
  RET->p.TYPENAME##var = in->TYPENAME##var;\
  RET->t = TYPENAME##type;\
}
...
static inline void fname(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(in1_TYPENAME)\
  in1_TYPENAME IN = in++->in1_TYPENAME##var;\
  in2_TYPENAME N = in++->in2_TYPENAME##var;\
  RET->p.in1_TYPENAME##var = IN OP N;\
  RET->t = in1_TYPENAME##type;\
}
#endif
...
#define __iec_(TYPENAME) \
static inline void SHL__BOOL__##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(BOOL)\
  BOOL IN = in++->BOOLvar;\
  TYPENAME N = in++->TYPENAME##var;\
  RET->p.BOOLvar = (N==0)? IN : __INIT_BOOL;\
  RET->t = BOOLtype;\
}
#endif
__ANY_INT(__iec_)
...
#define __iec_(TYPENAME) \
static inline void SHR__BOOL__##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(BOOL)\
  BOOL IN = in++->BOOLvar;\
  TYPENAME N = in++->TYPENAME##var;\
  RET->p.BOOLvar = (N==0)? IN : __INIT_BOOL;\
...
#define __iec_(TYPENAME) \
static inline void ROR__BOOL__##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(BOOL)\
  BOOL IN = in++->BOOLvar;\
  RET->p.BOOLvar = IN;\
  RET->t = BOOLtype;\
...
#else
#define __iec_(in1_TYPENAME,in2_TYPENAME) \
static inline void ROR__##in1_TYPENAME##__##in1_TYPENAME##__##in2_TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(in1_TYPENAME)\
  in1_TYPENAME IN = in++->in1_TYPENAME##var;\
  in2_TYPENAME N = in++->in2_TYPENAME##var;\
  RET->p.in1_TYPENAME##var = (IN >> N) | (IN << (8*sizeof(in1_TYPENAME)-N));\
  RET->t = in1_TYPENAME##type;\
}
#endif
__ANY_INT(__in1_anynbit_)
...
#define __iec_(TYPENAME) \
static inline void ROL__BOOL__##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(BOOL)\
  BOOL IN = in++->BOOLvar;\
  RET->p.BOOLvar = IN;\
  RET->t = BOOLtype;\
...
#else
#define __iec_(in1_TYPENAME,in2_TYPENAME) \
static inline void ROL__##in1_TYPENAME##__##in1_TYPENAME##__##in2_TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(in1_TYPENAME)\
  in1_TYPENAME IN = in++->in1_TYPENAME##var;\
  in2_TYPENAME N = in++->in2_TYPENAME##var;\
  N %= 8*sizeof(in1_TYPENAME);\
...
static inline void NOT_BOOL(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(BOOL)\
  RET->p.BOOLvar = !in->BOOLvar;\
  RET->t = BOOLtype;\
}
static inline void NOT__BOOL__BOOL(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(BOOL)\
  RET->p.BOOLvar = !in->BOOLvar;\
  RET->t = BOOLtype;\
}
#define __iec_(TYPENAME)\
static inline void NOT_##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(TYPENAME)\
  RET->p.TYPENAME##var = ~in->TYPENAME##var;\
  RET->t = TYPENAME##type;\
}
...
#undef __iec_
#define __iec_(TYPENAME)\
static inline void NOT__##TYPENAME##__##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(TYPENAME)\
  RET->p.TYPENAME##var = ~in->TYPENAME##var;\
  RET->t = TYPENAME##type;\
}
...
#define __iec_(TYPENAME)\
static inline void SEL_##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(TYPENAME)\
  BOOL G = in++->BOOLvar;\
  TYPENAME op0 = in++->TYPENAME##var;\
  TYPENAME op1 = in++->TYPENAME##var;\
  RET->p.TYPENAME##var = G ? op1 : op0;\
  RET->t = TYPENAME##type;\
}
__ANY(__iec_)
#undef __iec_
...
#define __iec_(TYPENAME)\
static inline void SEL__##TYPENAME##__BOOL__##TYPENAME##__##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(TYPENAME)\
  BOOL G = in++->BOOLvar;\
  TYPENAME op0 = in++->TYPENAME##var;\
  TYPENAME op1 = in++->TYPENAME##var;\
...
static inline void fname(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  ...
  TYPENAME op1 = in++->TYPENAME##var;\
  for (i = 1; i < count; i++){\
    TYPENAME tmp = in++->TYPENAME##var;\
    op1 = COND ? tmp : op1;\
  }\
  RET->p.TYPENAME##var = op1;\
  RET->t = TYPENAME##type;\
}
#endif
...
#define __iec_(TYPENAME)\
static inline void LIMIT_##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(TYPENAME)\
  TYPENAME MN = in++->TYPENAME##var;\
  TYPENAME IN = in++->TYPENAME##var;\
  TYPENAME MX = in++->TYPENAME##var;\
  RET->p.TYPENAME##var = IN > MN ? IN < MX ? IN : MX : MN;\
  RET->t = TYPENAME##type;\
}\
static inline void LIMIT__##TYPENAME##__##TYPENAME##__##TYPENAME##__##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(TYPENAME)\
  TYPENAME MN = in++->TYPENAME##var;\
  TYPENAME IN = in++->TYPENAME##var;\
  TYPENAME MX = in++->TYPENAME##var;\
...
static inline void LIMIT_##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  ...
  TYPENAME IN = in++->TYPENAME##var;\
  TYPENAME MX = in++->TYPENAME##var;\
  RET->p.TYPENAME##var = __time_cmp(IN, MN) > 0 ? /* IN>MN ?*/\
           __time_cmp(IN, MX) < 0 ? /* IN<MX ?*/\
           IN : MX : MN;\
  RET->t = TYPENAME##type;\
}\
...
static inline void LIMIT_STRING(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){
  TEST_EN(STRING)
  STRING MN = in++->STRINGvar;
  STRING IN = in++->STRINGvar;
  STRING MX = in++->STRINGvar;
  RET->p.STRINGvar = __STR_CMP(IN, MN) > 0 ? __STR_CMP(IN, MX) < 0 ? IN : MX : MN;
  RET->t = STRINGtype;\
}
static inline void LIMIT__STRING__STRING__STRING__STRING(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){
  TEST_EN(STRING)
  STRING MN = in++->STRINGvar;
  STRING IN = in++->STRINGvar;
  STRING MX = in++->STRINGvar;
...
//	ACTILITY : ATTENTION ICI K est après count  ##################

#define __iec_(in1_TYPENAME,in2_TYPENAME) \
static inline void MUX__##in2_TYPENAME##__##in1_TYPENAME##__##in2_TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  in1_TYPENAME K = in++->in1_TYPENAME##var;\
  TEST_EN_COND(in2_TYPENAME, K >= count)\
  in += K;\
  RET->p.in2_TYPENAME##var = in->in2_TYPENAME##var;\
  RET->t = in2_TYPENAME##type;\
}
...
static inline void fname(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  ...
  TYPENAME op1 = in++->TYPENAME##var;\
  TYPENAME op2 = in++->TYPENAME##var;\
  RET->p.TYPENAME##var = op1 != op2 ? 1 : 0;\
  RET->t = TYPENAME##type;\
}
...
static inline void fname(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  ...
  TYPENAME op1 = in++->TYPENAME##var;\
  TYPENAME op2 = in++->TYPENAME##var;\
  RET->p.BOOLvar = __time_cmp(op1, op2) != 0 ? 1 : 0;\
  RET->t = BOOLtype;\
}
...
static inline void fname(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  TEST_EN(ret_TYPENAME)\
  RET->p.ret_TYPENAME##var = OPERATION(in->par_TYPENAME##var);\
  RET->t = INTtype;\
}
#endif
...
#else
#define __left(TYPENAME) \
static inline void LEFT__STRING__STRING__##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  STRING IN = in++->STRINGvar;\
  TYPENAME L = in++->TYPENAME##var;\
  TEST_EN_COND(STRING, L < 0)\
  RET->p.STRINGvar = __INIT_STRING;\
  TYPENAME l = (TYPENAME)IN.len;\
  L = L < l ? L : l;\
  memcpy(&RET->p.STRINGvar.body, &IN.body, L);\
  RET->p.STRINGvar.len = L;\
  RET->t = STRINGtype;\
}
#endif
__ANY_INT(__left)
...
#else
#define __right(TYPENAME) \
static inline void RIGHT__STRING__STRING__##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  STRING IN = in++->STRINGvar;\
  TYPENAME L = in++->TYPENAME##var;\
  TEST_EN_COND(STRING, L < 0)\
  RET->p.STRINGvar = __INIT_STRING;\
  L = L < (TYPENAME)IN.len ? L : (TYPENAME)IN.len;\
  memcpy(&RET->p.STRINGvar.body, &IN.body[(TYPENAME)IN.len - L], L);\
  RET->p.STRINGvar.len = L;\
  RET->t = STRINGtype;\
}
#endif
...
#else
#define __mid(TYPENAME) \
static inline void MID__STRING__STRING__##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in){\
  STRING IN = in++->STRINGvar;\
  TYPENAME L = in++->TYPENAME##var;\
  TYPENAME P = in++->TYPENAME##var;\
  TEST_EN_COND(STRING, L < 0 || P < 0)\
  RET->p.STRINGvar = __INIT_STRING;\
  if(P <= (TYPENAME)IN.len){\
	P -= 1; /* now can be used as [index]*/\
	L = L + P <= (TYPENAME)IN.len ? L : (TYPENAME)IN.len - P;\
	memcpy(&RET->p.STRINGvar.body, &IN.body[P] , L);\
  	RET->p.STRINGvar.len = L;\
  }\
  RET->t = STRINGtype;\
}
...
#else
static inline void CONCAT(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in) {
  TEST_EN(STRING)
  UINT i;
  __strlen_t charcount;
  charcount = 0;
  ...
  for (i = 0; i < count && charcount < STR_MAX_LEN; i++)
  {
    STRING tmp = in++->STRINGvar;
    __strlen_t charrem = STR_MAX_LEN - charcount;
    __strlen_t to_write = tmp.len > charrem ? charrem : tmp.len;
    memcpy(&RET->p.STRINGvar.body[charcount], &tmp.body , to_write);
    charcount += to_write;
  }
  RET->p.STRINGvar.len = charcount;
  RET->t = STRINGtype;
}
#endif
...
#else
#define __iec_(TYPENAME) \
static inline void INSERT__STRING__STRING__STRING__##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in) {\
  STRING str1 = in++->STRINGvar;\
  STRING str2 = in++->STRINGvar;\
  TYPENAME P = in++->TYPENAME##var;\
  TEST_EN_COND(STRING, P < 0)\
  RET->p.STRINGvar = (STRING)__insert(str1,str2,P);\
  RET->t = STRINGtype;\
}
#endif
__ANY_INT(__iec_)
...
#else
#define __iec_(TYPENAME) \
static inline void DELETE__STRING__STRING__##TYPENAME##__##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in) {\
  STRING str = in++->STRINGvar;\
  TYPENAME L = in++->TYPENAME##var;\
  TYPENAME P = in++->TYPENAME##var;\
  TEST_EN_COND(STRING, L < 0 || P < 0)\
  RET->p.STRINGvar = (STRING)__delete(str,L,P);\
  RET->t = STRINGtype;\
}
#endif
__ANY_INT(__iec_)
...
#else
#define __iec_(TYPENAME) \
static inline void REPLACE__STRING__STRING__STRING__##TYPENAME##__##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in) {\
  STRING str1 = in++->STRINGvar;\
  STRING str2 = in++->STRINGvar;\
  TYPENAME L = in++->TYPENAME##var;\
  TYPENAME P = in++->TYPENAME##var;\
  TEST_EN_COND(STRING, L < 0 || P < 0)\
  RET->p.STRINGvar = (STRING)__replace(str1,str2,L,P);\
  RET->t = STRINGtype;\
}
#endif
__ANY_INT(__iec_)
...
#else
#define __iec_(TYPENAME) \
static inline void FIND__##TYPENAME##__STRING__STRING(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in) {\
  STRING str1 = in++->STRINGvar;\
  STRING str2 = in++->STRINGvar;\
  TEST_EN(TYPENAME)\
  RET->p.TYPENAME##var = (TYPENAME)__pfind(&str1,&str2);\
  RET->t = TYPENAME##type;\
}
#endif
__ANY_INT(__iec_)
...
/*** IEC-61131-3 Table 30 No 1b ***/
static inline void ADD_TIME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in) {
  TEST_EN(TIME)
  TIME IN1 = in++->TIMEvar;
  TIME IN2 = in++->TIMEvar;
  RET->p.TIMEvar = __time_add(IN1, IN2);
  RET->t = TIMEtype;
}

/*** IEC-61131-3 Table 30 No 2b ***/
static inline void ADD_TOD_TIME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in) {
  TEST_EN(TOD)
  TOD IN1 = in++->TODvar;
  TIME IN2 = in++->TIMEvar;
// *LNI* DataType inccorect
//  RET->p.TIMEvar = __time_add(IN1, IN2);
//  RET->t = TIMEtype;
  RET->p.TODvar = __time_add(IN1, IN2);
  RET->t = TODtype;
}

/*** IEC-61131-3 Table 30 No 3b ***/
static inline void ADD_DT_TIME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in) {
  TEST_EN(DT)
  DT IN1 = in++->DTvar;
  TIME IN2 = in++->TIMEvar;
// *LNI* DataType inccorect
//  RET->p.TIMEvar = __time_add(IN1, IN2);
//  RET->t = TIMEtype;
    RET->p.DTvar = __time_add(IN1, IN2);
    RET->t = DTtype;
}

/*** IEC-61131-3 Table 30 No 4b ***/
static inline void SUB_TIME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in) {
  TEST_EN(TIME)
  TIME IN1 = in++->TIMEvar;
  TIME IN2 = in++->TIMEvar;
  RET->p.TIMEvar = __time_sub(IN1, IN2);
  RET->t = TIMEtype;
}

/*** IEC-61131-3 Table 30 No 5b ***/
static inline void SUB_DATE_DATE(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in) {
  TEST_EN(TIME)
  DATE IN1 = in++->DATEvar;
  DATE IN2 = in++->DATEvar;
// *LNI* DataType inccorect
//  RET->p.DATEvar = __time_sub(IN1, IN2);
//  RET->t = DATEtype;
    RET->p.TIMEvar = __time_sub(IN1, IN2);
    RET->t = TIMEtype;
}

/*** IEC-61131-3 Table 30 No 6b ***/
static inline void SUB_TOD_TIME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in) {
  TEST_EN(TOD)
  TOD IN1 = in++->TODvar;
  TIME IN2 = in++->TIMEvar;
  RET->p.TODvar = __time_sub(IN1, IN2);
  RET->t = TODtype;
}

/*** IEC-61131-3 Table 30 No 7b ***/
static inline void SUB_TOD_TOD(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in) {
  TEST_EN(TIME)
  TOD IN1 = in++->TODvar;
  TOD IN2 = in++->TODvar;
  RET->p.TIMEvar = __time_sub(IN1, IN2);
...
/*** IEC-61131-3 Table 30 No 8b ***/
static inline void SUB_DT_TIME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in) {
  TEST_EN(DT)
  DT IN1 = in++->DTvar;
  TIME IN2 = in++->TIMEvar;
  RET->p.DTvar = __time_sub(IN1, IN2);
  RET->t = DTtype;
}

/*** IEC-61131-3 Table 30 No 9b ***/
static inline void SUB_DT_DT(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in) {
  TEST_EN(TIME)
  DT IN1 = in++->DTvar;
  DT IN2 = in++->DTvar;
  RET->p.TIMEvar = __time_sub(IN1, IN2);
...
/*** IEC-61131-3 Table 30 No 10b ***/
/***  MULTIME  ***/
#define __iec_(TYPENAME)\
static inline void MULTIME__TIME__TIME__##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in) {\
  TEST_EN(TIME)\
  TIME IN1 = in++->TIMEvar;\
  TYPENAME IN2 = in++->TYPENAME##var;\
  RET->p.TIMEvar = __time_mul(IN1, IN2);\
  RET->t = TIMEtype;\
}
__ANY_NUM(__iec_)
#undef __iec_
...
/***  MUL  ***/
#define __iec_(TYPENAME)\
static inline void MUL__TIME__TIME__##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in) {\
  TEST_EN(TIME)\
  TIME IN1 = in++->TIMEvar;\
  TYPENAME IN2 = in++->TYPENAME##var;\
  RET->p.TIMEvar = __time_mul(IN1, IN2);\
...
/*** IEC-61131-3 Table 30 No 11b ***/
/***  DIVTIME  ***/
#define __iec_(TYPENAME)\
static inline void DIVTIME__TIME__TIME__##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in) {\
  TEST_EN(TIME)\
  TIME IN1 = in++->TIMEvar;\
  TYPENAME IN2 = in++->TYPENAME##var;\
  RET->p.TIMEvar = __time_div(IN1, IN2);\
  RET->t = TIMEtype;\
}
__ANY_NUM(__iec_)
#undef __iec_
...
/***  DIV  ***/
#define __iec_(TYPENAME)\
static inline void DIV__TIME__TIME__##TYPENAME(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in) {\
  TEST_EN(TIME)\
  TIME IN1 = in++->TIMEvar;\
  TYPENAME IN2 = in++->TYPENAME##var;\
  RET->p.TIMEvar = __time_div(IN1, IN2);\
...
/*** IEC-61131-3 Table 30 No 12 ***/
/*** CONCAT_DATE_TOD ***/
static inline void CONCAT_DATE_TOD(EN_ENO_PARAMS, param_ret_t *RET, UINT count, param_u_t *in) {
  TEST_EN(DT)
  DATE IN1 = in++->DATEvar;
  TOD IN2 = in++->TODvar;
  RET->p.DTvar = __time_add(IN1, IN2);
