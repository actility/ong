...
{
	if	(strcmp(code,"STATUS_CONFLICT") == 0)	// TODO
		return	1;
	...
	int *level,int *cluster,int *num)
{
	...
	*num	= -1;
	...
	pt	= strstr(target,GetVar("w_driverpath"));
	if	(pt)
		target	= pt+strlen(GetVar("w_driverpath"));
...
void	*DiaUdpInit()
{
	...
	DiaClientInit(&callbacks);	// CB from diaclient.c
	DiaServerInit(&callbacks);	// CB from diaserver.c

	strcpy	(addr_str,GetCoapAddrL());
	strcpy	(port_str,GetCoapPortL());
//	dia_set_log_level(9);
	DiaCtxt = dia_createContext(addr_str, port_str, &callbacks);
	if (!DiaCtxt)
	{
