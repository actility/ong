...
char	*TbDesc[] = 
{ 
"11 01 0050 0002 00 4C 0006 02 0052 0054",
};
...
void	EventReport(char *addr,char *buf,int sz)
{
	t_zcl_msg	zmsg;
	int		ret;
	char		*zdata;
	...
	zmsg.m_cluster_id	= W_CLU_TICCBE;
	zmsg.m_attr_id		= W_ATT_TICCBE_DATA;
...
rtl_trace(0,"send attr tic type for sensor num=%d\n",numcap);
		zmsg->u_param.m_uint1_value.m_value	= 0x03;
		zmsg->m_cmd	= W_CMD_READ_ATTR_RESP;
	break;
	default :
		return;
...
void	DoClockSc()
{
	...
	{
/*
		sprintf	(buf,"%010d",nbclock);
		memset(buf,'A',sizeof(buf));
		buf[sizeof(buf)-1]	= '\0';
*/

		sz	= sizeof(buf);
		AwStrToBin(data,(unsigned char *)buf,&sz);
		EventReport(ReportAddress,buf,sz);
	}
	nbclock++;
