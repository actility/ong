...
char	*DescBinary = "11 01 0050 0002 00 4C 0003 01 000F";
...
void	EventReportValue(char *addr,int value)
{
	t_zcl_msg	zmsg;
	int		ret;
	char		*zdata;
	...
	zmsg.m_cluster_id	= W_CLU_BINARYINPUT;
	zmsg.m_attr_id		= W_ATT_BINARYINPUT_CUR_VALUE;
	zmsg.u_param.m_uint1_value.m_value	= value;
...
void	EventReportCounter(char *addr,u_int value)
{
	t_zcl_msg	zmsg;
	int		ret;
	char		*zdata;
	...
	zmsg.m_cmd		= W_CMD_REPO_ATTR_EVNT;
	zmsg.m_cluster_id	= W_CLU_BINARYINPUT;
	zmsg.m_attr_id		= W_ATT_BINARYINPUT_COUNTER;
	zmsg.u_param.m_uint4_value.m_value	= value;
...
int	ReportSim(t_sim *sim,time_t now)
{
	...
	EventReportValue(sim->sm_addr,sim->sm_value);
	EventReportCounter(sim->sm_addr,sim->sm_counter);

	sim->sm_nextreport	= sim->sm_nextreport + (sim->sm_mintime*1000);

	sim->sm_value	= sim->sm_value ^ 1;
	sim->sm_counter++;
