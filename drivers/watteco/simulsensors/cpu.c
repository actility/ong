...
//char	*ReportAddress	= "1-cpuload@aaaa::1";
char	*ReportAddress	= "1-cpuload@::1";
...
//	rtl_trace(0,"DoClockSc() cpuload=%d last=%d\n",cpuload,last);

	if	((nbclock % 30) == 0 || ABS(last - cpuload) >= 5)
	{

		rtl_trace(1,"cpuload=%d last=%d\n",cpuload,last);
		EventReport(ReportAddress,cpuload);
	}
	last	= cpuload;
	nbclock++;
