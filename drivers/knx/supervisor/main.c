...
char *knownNetworksPath = NULL;

NetworkList_t NetworkList;
...
{
    RTL_TRDBG(5, "receive timer cl=%d ty=%d vsize=%ld\n", imsg->im_class, imsg->im_type, rtl_vsize(getpid()));
...
Usage(char *prg, char *fmt)
{
    ...
    printf("    -i        : reset KNX networks at start-up (if not provided the\n");
    printf("                HAN are resumed from previous run)\n");
    printf("    --version : print the version string of the KNX driver\n");
...
/**
 * Detect that child die (i.e. EIBD process)
 */
void
catchChild(int sig)
{
...
/**
 * @brief Delete all known networks definition files.
 * This operation is done when resetting the HAN.
 * @param path the path to the known-networks files.
 */
static void 
deleteKnownNetworks(char *path)
{
    void *dir;
    char *name;
    char *point;
...
int main(int argc,char *argv[])
{
    ...
    if (!Operating) {
        sprintf(tmp, "%s/networks", getenv("AW_DATA_DIR"));
    } else {
        sprintf(tmp, "%s/usr/data/%s/networks", rootactPath, GetAdaptorName());
    }
    rtl_mkdirp(tmp);
    knownNetworksPath = malloc(strlen(tmp) + 1);
    sprintf(knownNetworksPath, "%s", tmp);
    ...
    if (resetHanAtReboot)
    {
      // delete network topology description files
      deleteKnownNetworks(knownNetworksPath);
    }
    ...
    // Start KNX stuff here
    ret = RangeLoadDatabase();
    if (ret != 0) {
        RTL_TRDBG(TRACE_ERROR, "RangeLoadDatabase failed\n");
        exit(0);
    }
    
    ret = DptLoadDatabase();
    if (ret != 0) {
        RTL_TRDBG(TRACE_ERROR, "DptLoadDatabase failed\n");
        exit(0);
    }
    
    ret = ManufacturerLoadDatabase();
    if (ret != 0) {
        RTL_TRDBG(TRACE_ERROR, "ManufacturerLoadDatabase failed\n");
        exit(0);
    }
    
    SensorInitNetworkList();
    ...
    // Create the KNX IPU, need to create Network from the ONG
    ...
    RTL_TRDBG(0,"Terminate all EIBD\n");
    EibdCleanupAll();
