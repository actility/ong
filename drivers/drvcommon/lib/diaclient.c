...
unsigned	int		DiaCreaCount;
unsigned	int		DiaRetrCount;
unsigned	int		DiaUpdaCount;
unsigned	int		DiaDeleCount;
...
unsigned	int	DiaDefaultRequestMode	= 0;
...
int WDiaCreateRequest (char *reqname, char *reqEntity, char *targetID, void *ctt, int len, char *cttType, void *optAttr, void *optHeader, int *tid)
{
	...
	if	(ret >= 0 && DiaTraceRequest)
		WDiaTraceRequest(CLT_SIDE_DIA_C,*tid,"",reqname,reqEntity,
				targetID,ctt,len,cttType);
	return	ret;
}
...
int WDiaUpdateRequest (char *reqname, char *reqEntity, char *targetID, void *ctt, int len, char *cttType, void *optAttr, void *optHeader, int *tid)
{
	...
	if	(ret >= 0 && DiaTraceRequest)
		WDiaTraceRequest(CLT_SIDE_DIA_U,*tid,"",reqname,reqEntity,
				targetID,ctt,len,cttType);
	return	ret;
}
	...
	void *optAttr, void *optHeader, int *tid)
{
	...
	if	(ret >=0 && DiaTraceRequest)
		WDiaTraceRequest(CLT_SIDE_DIA_R,*tid,"",reqname,reqEntity,
				targetID,NULL,0,"");
	return	ret;
}
	...
	void *optAttr, void *optHeader, int *tid)
{
	...
	if	(ret >=0 && DiaTraceRequest)
		WDiaTraceRequest(CLT_SIDE_DIA_D,*tid,"",reqname,reqEntity,
			targetID,NULL,0,"");
	return	ret;
}
...
drop :
	...
	for	(i = 0 ; i < MAX_REQUEST_DIA ; i++)
	{
		if	(TbReq[i].rq_waitRsp == NUSE_REQUEST_DIA)
		{
			...
			req->rq_initAt	= rtl_timemono(NULL);
...
{
	DiaDefaultRequestMode	= flags;
}

void	DiaSetRequestMode(int flags)
{
	DiaRequestMode	= flags | DiaDefaultRequestMode;
}
...
void	WDiaAddRequest(t_dia_req *req)
{
	...
	req->rq_sendAt	= rtl_timemono(NULL);
