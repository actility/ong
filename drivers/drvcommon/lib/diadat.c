...
void DIA_LEAK WDiaUCBRequestOk_DiaDatDelOk(t_dia_req *preq, t_cmn_sensor *cmn) {}
...
static	void	DiaDatDelOk(t_dia_req *preq,t_dia_rspparam *par)
{
	DIA_DEVICE_CONTEXT();

	RTL_TRDBG(2,"OK DIA DAT delete '%s' num=%04x attr=%04x member=%d\n",
	cmn->cmn_ieee,preq->rq_cluster,preq->rq_attribut,preq->rq_member);

	cmn->cmn_dia_datok	= 0;

	WDiaUCBRequestOk(DIA_FUNC,preq,cmn);
	WDiaUCBRequestOk_DiaDatDelOk(preq,cmn);
}
...
void	DiaDatContRetrieve(t_dia_req *preq,t_dia_rspparam *par,...)
{
	DIA_LOCAL_VAR();
	DIA_DEVICE_CONTEXT();
...
if	(PseudoOng)
	...
	// REQUEST
retry	:
	DIA_DELETE_REQUEST(DiaDatContDelete);
return;
	}
	
	// RESPONSE
	DIA_DELETE_RESPONSE(DiaDatError,DiaDatError,DiaDatDelOk);
}
